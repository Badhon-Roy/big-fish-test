...
function useStyleDecals(colors: any) {
  return useMemo(() => {
    ...
    if (colors.collarType === "Round") {
      ...
      // 1. Draw the white triangular chest insert (as seen in user's image)
...
interface PoloCollar3DProps {
  colors: any;
  patternFront: THREE.Texture | null;
  fabricConfig: {
    roughness: number;
...
function PoloCollar3D({
  ...
  // Dynamic canvas texture for the white collar with orange border stripe
  const collarTexture = React.useMemo(() => {
    const S = 1024;
    const cv = document.createElement("canvas");
    cv.width = S;
    cv.height = S;
    ...
    // 1. Fill base collar body (White)
    ctx.fillStyle = baseColor;
    ctx.fillRect(0, 0, S, S);

    // 2. Draw the offset border stripe (orange/trim)
    // V = 1 is the outer edge (top of canvas, y = 0)
    // U = 0 is left edge (x = 0), U = 1 is right edge (x = S)
    const border = Math.round(S * 0.06); // Offset from edge
    const thickness = Math.round(S * 0.045); // Stripe thickness
    ...
    ctx.beginPath();
    ctx.moveTo(border + thickness / 2, S);
    ctx.lineTo(border + thickness / 2, border + thickness / 2);
    ctx.lineTo(S - (border + thickness / 2), border + thickness / 2);
    ctx.lineTo(S - (border + thickness / 2), S);
    ctx.stroke();
  ...
  const baseMaterial = React.useMemo(() => {
    return new THREE.MeshStandardMaterial({
      color: "#ffffff",
      map: collarTexture || undefined,
      roughness: 1.0, // forced to 1.0 for matte jersey fabric appearance
      metalness: 0.0, // forced to 0.0 to eliminate glossy reflection
      normalMap: fabricConfig.normalMap || undefined,
      normalScale: fabricConfig.normalScale,
      envMapIntensity: 0.25,
    });
  }, [collarTexture, fabricConfig]);
...
function Jersey3D({
  ...
  const alignedGeometry = React.useMemo(() => {
    ...
    const targetHeight = 0.61298;

    const targetCenterX = -0.00075;
    ...
    // Scale geometry uniformly to match target height
    geom.scale(uniformScale, uniformScale, uniformScale);

    // Translate geometry to target center coordinates
    geom.translate(targetCenterX, targetCenterY, targetCenterZ);
                      ...
                      {[
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            // Auto toggle collar logic for aesthetic presets
