...
export function JerseySVG({
  ...
  fusionDots.push(<polygon key="solid-back" points="66,0 100,0 100,100 76,100" fill={secondary} />);
  ...
  for (let y = 98; y >= 30; y -= 3) {
    ...
    horizonLines.push(<line key={y} x1="0" y1={y} x2="100" y2={y} stroke={secondary} strokeWidth={thickness} />);
  }
  ...
  novaRows.forEach(({ c, r }, rowIdx) => {
    for (let t = 0; t <= c; t += 4.5) {
      ...
      if (y >= 0 && y <= 100) {
        novaDots.push(
          <circle key={`nd-${rowIdx}-${t.toFixed(1)}`} cx={x} cy={y} r={r} fill={secondary} />
        );
      }
    }
  });
  ...
  for (let x = 4; x <= 96; x += 4) {
    for (let y = 96; y >= 40; y -= 4) {
      ...
      if (rand < (y - 35) / 65) {
        ...
        matrixBlocks.push(<rect key={`${x}-${y}`} x={x - sz / 2} y={y - sz / 2} width={sz} height={sz} fill={secondary} />);
      }
    }
  }
  ...
  for (let x = 28; x <= 72; x += 4) {
    ...
    zenithLines.push(<line key={x} x1={x} y1="10" x2={x} y2="80" stroke="url(#zenithGrad)" strokeWidth="2" strokeLinecap="round" opacity={opacity} />);
  }
  ...
  const patterns: Record<string, JSX.Element> = {
    ...
    animal: (
      <>
        <path d="M0,0 L100,0 L100,50 Q75,80 50,55 Q25,80 0,50 Z" fill={secondary} />
      </>
    ),
    avatar: (
      <>
    ...
    apex: (
      <>
        ...
        {/* Main V Chevron */}
        <polygon points="0,33 50,53 100,33 100,45 50,65 0,45" fill={secondary} />
        {/* Shadow Chevrons */}
        <polygon points="0,47 50,67 100,47 100,57 50,77 0,57" fill={secondary} opacity="0.2" />
        <polygon points="0,59 50,79 100,59 100,69 50,89 0,69" fill={secondary} opacity="0.1" />
      </>
    ),
    bolt: (
      <>
    ...
    edge: (
      <>
        <polygon points="0,0 50,0 50,10 46,10 46,20 50,20 50,30 46,30 46,40 50,40 50,50 46,50 46,60 50,60 50,70 46,70 46,80 50,80 50,90 46,90 46,100 0,100" fill={secondary} />
      </>
    ),
    fusion: (
      <>
    ...
    summit: (
      <>
        ...
        <polygon points="0,85 15,85 25,78 23,80 35,71 33,73 50,60 67,73 65,71 77,80 75,78 85,85 100,85 100,100 0,100" fill={secondary} />

        {/* Hollow chevron peak 2 */}
        <polygon points="20,78 32,65 30,67 42,54 40,56 50,49 60,56 58,54 68,67 66,65 80,78 80,82 66,71 68,69 58,60 60,58 50,54 40,58 42,60 32,69 30,71 20,82" fill={secondary} />

        {/* Hollow chevron peak 3 */}
        <polygon points="25,70 36,58 34,60 50,42 66,60 64,58 75,70 75,74 64,62 66,60 50,47 34,60 36,62 25,74" fill={secondary} />
    ...
    titan: (
      <>
        ...
        {/* Left side stripe */}
        <polygon points="41,0 43,0 43,60 41,64 41,100 37,100 37,64 41,60" fill={primary} />
        {/* Right side stripe */}
        <polygon points="57,0 59,0 59,60 63,64 63,100 59,100 59,64 57,60" fill={primary} />
